...
#pragma warning restore CS0168
        ...
        {
            FormConfig dConfig = Genie4Lib.MyResources.FormResource("FormConfig", true) as FormConfig;
            if (dConfig.Visible == false | TextBoxInput.Focused == true)
            {
        ...
        private void FormMain_Load(object sender, EventArgs e)
        {
            ...
            {
                (Genie4Lib.MyResources.FormResource("DialogChangelog", true) as DialogChangelog).ShowDialog(this);
            }
            /* TODO ERROR: Skipped EndIfDirectiveTrivia */
            TextBoxInput.Focus();
            InitWorkerThread();
        ...
        private void AddScriptToToolStrip(Script oScript)
        {
            ...
            if (Monitor.TryEnter(ToolStripButtons.Items))
            {
                try
                {
                    ...
                    oScriptButton.Text = "";
                    oScriptButton.Image = Genie4Lib.MyResources.ImageResource("control_play");
                    ...
                    foreach (object oItem in ToolStripButtons.Items)
                    {
                        if (oItem is ToolStripSplitButton)
                        {
                            if (((ToolStripSplitButton)oItem).Tag is Script && (((Script)((ToolStripSplitButton)oItem).Tag).ScriptID ?? "") == (sender.ScriptID ?? ""))
                            {
                                ...
                                {
                                    ((ToolStripSplitButton)oItem).Image = Genie4Lib.MyResources.ImageResource("control_pause");
                                }
                                else if (state == Script.ScriptState.running)
                                {
                                    ((ToolStripSplitButton)oItem).Image = Genie4Lib.MyResources.ImageResource("control_play");
                                }
...
#pragma warning restore CS0168
        ...
        private void UpdateMainWindowTitle()
        {
            ...
            strTitle += "Genie " + Genie4Lib.MyResources.GetApplicationVersion();
            Text = strTitle;
        }
        ...
        {
            DialogConnect dConnect = Genie4Lib.MyResources.FormResource("DialogConnect", true) as DialogConnect;
            if (dConnect.ShowDialog(this) == DialogResult.OK)
            {
                string argsAccountName = dConnect.AccountName;
                string argsPassword = dConnect.Password;
                string argsCharacter = dConnect.Character;
                string argsGame = dConnect.Game;
                ConnectToGame(argsAccountName, argsPassword, argsCharacter, argsGame);
                SavePasswordToolStripMenuItem.Checked = dConnect.CheckBoxSavePassword.Checked;
            }
        }
        ...
        {
            FormConfig fConfig = Genie4Lib.MyResources.FormResource("FormConfig", true) as FormConfig;
        ...
        {
            DialogException dException = Genie4Lib.MyResources.FormResource("DialogException", true) as DialogException;

            if (dException.Visible == false)
            {
                ...
                dException.Show(this, section + System.Environment.NewLine + message + System.Environment.NewLine + System.Environment.NewLine + description);
            }
        }
        ...
        private void ShowDialogAutoMapperException(string section, Exception ex)
        {
            ...
            if (dException.Visible == false)
            {
                ...
                dException.Show(this, sbDetails.ToString(), "There was an unexpected error in the AutoMapper. This may be due to a programming bug.");
            }
        }
        ...
        {
            ScriptExplorer dExplorer = Genie4Lib.MyResources.FormResource("ScriptExplorer", true) as ScriptExplorer;
            if (dExplorer.Visible == false)
            {
                dExplorer.MdiParent = this;
                dExplorer.Globals = m_oGlobals;
                dExplorer.EventRunScript += ClassCommand_RunScript;
                dExplorer.Show();
            }

            dExplorer.BringToFront();
        }
        ...
        {
            DialogProfileConnect dConnect = Genie4Lib.MyResources.FormResource("DialogProfileConnect", true) as DialogProfileConnect;

            dConnect.ConfigDir = m_oGlobals.Config.ConfigDir;
            dConnect.ClassicConnect = m_oGlobals.Config.bClassicConnect;
            if (dConnect.ShowDialog(this) == DialogResult.OK)
            {
                m_sCurrentProfileFile = string.Empty;
                LoadProfile(dConnect.ProfileName + ".xml", true);
            }
        }
        ...
        private void LoadProfile(string FileName, bool DoConnect = false)
        {
            ...
            if (m_oProfile.LoadFile(FileName) == true)
            {
                ...
                if (DoConnect == true)
                {
                    ...
                    {
                        DialogConnect dConnect = Genie4Lib.MyResources.FormResource("DialogConnect", true) as DialogConnect;
                        dConnect.AccountName = sAccount;
                        dConnect.Password = "";
                        dConnect.Character = sCharacter;
                        dConnect.Game = sGame;
                        if (dConnect.ShowDialog(this) == DialogResult.OK)
                        {
                            string argsAccountName = dConnect.AccountName;
                            string argsPassword1 = dConnect.Password;
                            string argsCharacter = dConnect.Character;
                            string argsGame = dConnect.Game;
        ...
        {
            Interaction.Shell("explorer.exe " + m_oGlobals.Config.LogDir, AppWinStyle.NormalFocus, false);
        }
